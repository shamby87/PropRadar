...
def getBestPlays():
    ...
    # popWeight = 0
    # advWeight = 1
    minAdv = 0.99 # Ignore plays with a worse adv than this
    ...
    for i in range(len(filtered)):
        ...
        for option in player['options']:
            players[stat][name][option['outcome']] = { # 'Outcome' is over or under
                ...
                'playerId': option['subject_id'],
                'team': option['subject_team'],
                'sport': option['sport'],
    ...
    #                             for option in p.values() where option['']
    #              )
    # utils.logMsg(json.dumps(players, indent=2))
        ...
        for player in players[sleeper_stat]:
            for outcome in ['over', 'under']:
                if len(players[sleeper_stat][player][outcome]['otherBooks']) > 1:
                    ...
                    pop = players[sleeper_stat][player][outcome]['pop']
                    players[sleeper_stat][player][outcome]['avgAdvantage'] = adv
                    if adv < minAdv: # ignore
                        players[sleeper_stat][player][outcome]['avgAdvantage'] = -1 
                        players[sleeper_stat][player][outcome]['pop'] = -1
                        continue
    ...
    for sleeper_stat in sleeper_stats:
        for player in players[sleeper_stat]:
            for outcome in ['over', 'under']:
                ...
                if len(players[sleeper_stat][player][outcome]['otherBooks']) < 3:
                    players[sleeper_stat][player][outcome]['fitness'] = 0
                else:
                    # players[sleeper_stat][player][outcome]['fitness'] = popWeight*popScale + advWeight*advScale
                    players[sleeper_stat][player][outcome]['fitness'] = advScale

    top_plays = {}
    # After all is done, utils.logMsg the top plays for each stat line
    for sleeper_stat in sleeper_stats:
        # Sort by greatest fitness, only look at better of over/under for each player 
        utils.logMsg(f"{sleeper_stat}:")
        result = dict(sorted(players[sleeper_stat].items(), key=lambda item: max(item[1]['over']['fitness'], item[1]['under']['fitness']), reverse=True))

        num = min(6, len(result)) # Only utils.logMsg top 6 plays max
        for r in result.items():
            if num > 0:
                ...
                utils.logMsg(f"\t{r[0]} - line: {ou} {info['line']}, avg advantage: {round(info['avgAdvantage'], 4)}, len: {len(info['otherBooks'])}, popularity: {info['popularity']}, fitness: {round(info['fitness'], 4)}")
                
                # Add this play to our potential best parlay if this is the best line for the player
                if (r[0] not in top_plays) or top_plays[r[0]]['fitness'] < info['fitness']:
                    top_plays[r[0]] = {'ou': ou, 'payout': info['payout'], 'line': info['line'], 'avgAdvantage': info['avgAdvantage'], 'len': len(info['otherBooks']), 'popularity': info['popularity'], 'SleeperStat': sleeper_stat, 'fitness': info['fitness'], 'lineId': info['lineId'], 'team': info['team'], 'playerId': info['playerId'], 'sport': info['sport']}
            
            num -= 1

    # utils.logMsg out the full best play
    result = dict(sorted(top_plays.items(), key=lambda item: item[1]['fitness'], reverse=True))
    num = min(6, len(result)) # Only utils.logMsg top 6 plays max
    utils.logMsg("\nFull best play:")
    for r in result.items():
        if num > 0:
            utils.logMsg(f"\t{r[0]} - line: {r[1]['ou']} {r[1]['line']} {r[1]['SleeperStat']}, payout: {r[1]['payout']}x, avg advantage: {round(r[1]['avgAdvantage'], 4)}, len: {r[1]['len']}, popularity: {r[1]['popularity']}, fitness: {round(r[1]['fitness'], 4)}")
